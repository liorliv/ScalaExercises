...
object Anagrams {
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] =
    dictionary.groupBy(wordOccurrences).withDefaultValue(List.empty)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    ...
    case (char, count) :: rest =>
      val subsets = combinations(rest) // Generate subsets for the remaining occurrences
      subsets ++ (for {
        subset <- subsets
        n <- 1 to count
      } yield (char, n) :: subset) // Add subsets with varying counts of the current character
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    x.map { case (char, count) =>
      val remainingCount = count - y.toMap.getOrElse(char, 0)
      (char, remainingCount)
    }.filter(_._2 > 0)
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def subcombine(occ: Occurrences): List[Sentence] = {
      if (occ.isEmpty) List(List.empty)
      else
        for {
