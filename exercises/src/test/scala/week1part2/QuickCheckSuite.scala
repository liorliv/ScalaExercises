...
class QuickCheckSuite extends munit.FunSuite {
  ...
  test("Binomial heap satisfies properties. (5pts)") {
    assert(
      check(asProp(new QuickCheckHeap with week1part2.test.BinomialHeap))(identity).passed
    )
  }

  test("Bogus (1) binomial heap does not satisfy properties. (10pts)") {
    checkBogus(new QuickCheckHeap with week1part2.test.Bogus1BinomialHeap)
  }

  test("Bogus (2) binomial heap does not satisfy properties. (10pts)") {
    checkBogus(new QuickCheckHeap with week1part2.test.Bogus2BinomialHeap)
  }

  test("Bogus (3) binomial heap does not satisfy properties. (10pts)") {
    checkBogus(new QuickCheckHeap with week1part2.test.Bogus3BinomialHeap)
  }

  test("Bogus (4) binomial heap does not satisfy properties. (10pts)") {
    checkBogus(new QuickCheckHeap with week1part2.test.Bogus4BinomialHeap)
  }

  test("Bogus (5) binomial heap does not satisfy properties. (10pts)") {
    checkBogus(new QuickCheckHeap with week1part2.test.Bogus5BinomialHeap)
  }
